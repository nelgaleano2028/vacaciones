...
//Datos del Empleado
$qry2="select c.SEXO, a.COD_CTG, NOM_EPL,APE_EPL,CEDULA,SAL_BAS,NOM_CAR,TO_CHAR(INI_CTO,'DD-MM-YYYY')as INI_CTO, TO_CHAR(FEC_RET,'DD-MM-YYYY')as FEC_RET, TO_CHAR(c.fec_ant_emp_gru,'DD-MM-YYYY')as FECHA_CESION 
from empleados_basic a, cargos b, empleados_gral c where a.cod_car=b.cod_car and a.cod_epl=c.cod_epl and a.cod_epl='".$codigoemple."'";
////SE LE AGREGO AL QUERY FECHA_CESION(SI NO TIENE REGISTRO EN ESE CAMPO, MUESTRA FEC_RET) ////
...
$cesion=''.$row2["FECHA_CESION"].''; ////VARIABLE QUE CONTIENE LA FECHA_CESION////

if (empty($cesion)) {
    $f_cesion=''.$row2["FEC_RET"].'';	//// LA VARIABLE $f_cesion ES LA QUE REEMPLAZA LA VARIABLE $fecha_retiro ////
}else{
	$f_cesion=''.$row2["FECHA_CESION"].'';	
}//// CONDICION QUE VALIDA LAS FECHAS ////	
...
$qry32="select TO_CHAR(FEC_ANT_EMP_GRU,'DD-MM-YYYY') as VTO_CTO from empleados_gral where cod_epl='".$codigoemple."'";
