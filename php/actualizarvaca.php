...
$qry2="update ausencias_tmp set fec_ini= TO_DATE ('$fecha_ini', 'MM-DD-YY'), fec_fin=TO_DATE ('$fecha_fin', 'MM-DD-YY'),dias=$dias, fec_ini_r=TO_DATE ('$fecha_ini', 'MM-DD-YY'), fec_fin_r=TO_DATE ('$fecha_fin', 'MM-DD-YY') where CNSCTVO='".$numero."'";
...
$qry3="select b.cedula as CEDULA ,a.cod_jefe as COD_JEFE, b.nom_epl as NOM_EPL, b.ape_epl as APE_EPL, a.email as EMAIL from empleados_gral a, empleados_basic b where a.cod_epl='".$codigo."' and a.cod_epl=b.cod_epl";
...
$query5 = "select b.NOM_EPL AS JEFE , b.APE_EPL AS APEJEFE, a.COD_JEFE AS COD_JEFE, a.email as EMAIL, b.cedula as CEDULA from empleados_gral a, empleados_basic b WHERE a.COD_EPL = b.COD_EPL and a.COD_EPL = '$cod_jefe'";
...
if($cod_jefe==NULL){
	...
	$contenido1="<p align='justify'><br>".$nombre." ".$apellido.", de acuerdo con la solicitud de su jefe, las fechas de vacaciones fueron editadas exitosamente, su nueva fecha es desde el ".date("d/m/Y",strtotime($fecha_ini))." hasta ".date("d/m/Y",strtotime($fecha_fin)).", ".$dias." días hábiles.<br><br> 
En ".$empresa_real." vivimos la mejor experiencia de construir el mejor lugar para trabajar en Colombia.
<br>&nbsp;<br>
Este mensaje es informativo por favor no dar respuesta a esta cuenta de correo. Si tienes alguna duda u observación crea tu llamada a la Mesa Centro de Servicios Compartidos haciendo <a href='http://intranet/MesasAyuda/Control.aspx?idSer=SERVICIOS DE NOMINA&idSSer=VACACIONES&pt=NOVEDADES VACACIONES&id=2' style='color: #770003'>clic aquí</a>.
...
Muchas Gracias por utilizar este servicio.<br>&nbsp;<br></p>";
	...
	$contenido2="<p align='justify'><br>De acuerdo con su solicitud se le han modificado las fechas de vacaciones a ".$nombre." ".$apellido.", su nueva fecha es desde el ".date("d/m/Y",strtotime($fecha_ini))." hasta ".date("d/m/Y",strtotime($fecha_fin)).", ".$dias." días hábiles.<br>&nbsp;<br>
	En ".$empresa_real." vivimos la mejor experiencia de construir el mejor lugar para trabajar en Colombia.
...
Muchas Gracias por utilizar este servicio.<br>&nbsp;<br></p>";
	...
	$mail= new mailer();
         ...
         $mail->Subject = "Vacaciones Programadas"; // Este es el titulo del email.
           //-----FIN EMAIL-----
	   ...
	   $sqlmail = "INSERT INTO t_admail (CEDULA, NOMBRES, APELLIDOS, FECHA_REG, NOVEDAD, COMENTARIO, EMPRESA) VALUES ('".$ced_jefe."','".$nom_jefe."','".$ape_jefe."',SYSDATE,'Vacaciones','".$email."','".$empresamail."')";
$conn->Execute($sqlmail);
        ...
        $mail->AddAddress($email_epl); // Esta es la dirección a donde enviamos $email_epl
         ...
         $mail->Subject = "Confirmacion de Vacaciones Programadas"; // Este es el titulo del email.
           //-----FIN EMAIL-----
         ...
         $mail->Send(); // Envía el correo.
	 	 ...
	 	 ///SE INSERTA EL CONTROL DE ENVIO DE CORREOS
	   $sqlmail = "INSERT INTO t_admail (CEDULA, NOMBRES, APELLIDOS, FECHA_REG, NOVEDAD, COMENTARIO, EMPRESA) VALUES ('".$cedula_epl."','".$nombre."','".$apellido."',SYSDATE,'Vacaciones','".$email_epl."','".$empresamail."')";
$conn->Execute($sqlmail);
