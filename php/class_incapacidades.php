...
class vacaciones{
    ...
    public function vacaciones_email(){
            ...
            try{
                ...
                $sql="--SQL PARA RESPONDER SOLICITUD VIA EMAIL--
                     select au.cod_epl AS COD_EPL, epl.nom_epl as NOM_EPL, epl.cedula as CEDULA, epl.ape_epl AS APE_EPL,
                     au.fec_ini AS FEC_INI,au.fec_fin AS FEC_FIN,
                     au.dias AS DIAS,au.observacion AS OBSERVACIONES,au.cnsctvo as CONSECUTIVO
                     from empleados_basic epl, incapacidades_tmp au  
    ...
    public function aceptar_vacaciones(){
            ...
            try{
            global $conn,$odbc;
            $conse="select count(*)+1 as CONSECUTIVO from incapacidades_tmp";
            $ver=$conn->Execute($conse);
             $fila=@$ver->FetchRow();
             $consecutivo=$fila["CONSECUTIVO"];
            //if($fila["nom_admin"] != $this->usuario){
            if($odbc == "oci8"){
            $sql="insert into incapacidades_tmp (cod_con,cod_epl, fec_ini, fec_fin, estado, dias,cod_aus, fec_ini_r, fec_fin_r,cnsctvo)
    ...
    private function cambia_estado_conf(){
            ...
            $sql="update incapacidades_tmp set estado='C', cod_encardo='".$this->encargado."' where cod_epl='".$this->cod_epl."' and cnsctvo='".$this->consecutivo."'";
            $rs=$conn->Execute($sql);
            if($rs){
                $res = true;
            }
       ...
       public function cambia_estado_cance(){
            ...
            $sql="update incapacidades_tmp set estado='R', observacion='".$this->observacion."', cod_encardo='".$this->encargado."', RESPUESTA_POR = 'Jefe' where cnsctvo='".$this->consecutivo."' and cod_epl='".$this->cod_epl."'";
            $rs=$conn->Execute($sql);
            if($rs){
                $res = true;
            }
...
au.fec_solicitud AS FECHASOL,
	...
	public function solicitud_epl_gral_fecha(){
...
au.fec_solicitud AS FECHASOL,
            ...
            and emp.cod_cc2=cen.cod_cc2
			and au.estado = 'P'
			and to_char(au.fec_ini,'MM') = TO_CHAR(SYSDATE,'MM')
			and to_char(au.fec_ini,'YY') = TO_CHAR(SYSDATE,'YY')
            order by fec_ini desc";
...
au.CNSCTVO as CONSECUTIVO,
            ...
            from empleados_basic emp, cargos car, centrocosto2 cen ,incapacidades_tmp au
...
from incapacidades_tmp a, empleados_basic e, cargos c, periodos p, centrocosto2 cc
    ...
    //vacaciones por area
       public function solicitud_epl_jefe(){
         ...
         //Sentencia sql de solicitud de vacaciones de empleado por jefe
            ...
            from empleados_basic emp, cargos car, centrocosto2 cen ,incapacidades_tmp au,empleados_gral gral
      ...
      //solicitud de vacaciones rechazadas por jefe
        ...
        public function solicitud_epl_jefe_rechazada(){
         ...
         //pendiente
            ...
            from empleados_basic emp, cargos car, centrocosto2 cen ,incapacidades_tmp au,empleados_basic emp2, empleados_gral gral
...
";
     ...
     /*
         ...
         *
        */
        public function mensaje_solicitud($contenido,$portal,$youtube,$facebook,$twitter,$titulo,$imagen='http://i60.tinypic.com/vaubr.jpg'){ 
