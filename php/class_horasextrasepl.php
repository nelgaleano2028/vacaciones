...
class vacaciones{
    ...
    public function vacaciones_email(){
            ...
            try{
                ...
                $sql="--SQL PARA RESPONDER SOLICITUD VIA EMAIL--
                     select au.cod_epl AS COD_EPL, epl.cedula as CEDULA, epl.nom_epl as NOM_EPL,epl.ape_epl AS APE_EPL,
    ...
    public function aceptar_vacaciones(){
            ...
            $sql="update horasextras_tmp set estado='L', cod_encardo='".$this->encargado."', RESPUESTA_POR = '$respuesta' where cnsctvo='".$this->consecutivo."' and cod_epl ='".$this->cod_epl."'";
    ...
    private function cambia_estado_conf(){
       ...
       public function cambia_estado_cance(){
          ...
          $respuesta = $_SESSION['nombre'].' '. $_SESSION['ape'];
            $sql="update horasextras_tmp set estado='R', observacion='".$this->observacion."', cod_encardo='".$this->encargado."', RESPUESTA_POR = '$respuesta' where COD_EPL = '".$this->cod_epl."' and cnsctvo='".$this->consecutivo."'";
            $rs=$conn->Execute($sql);
            if($rs){
                $res = true;
            }
    ...
    public function solicitud_epl_gral(){
		 ...
		 $idciden = $_SESSION['cc'];
            $sql="select a.cod_con AS COD_CON, e.cod_epl AS COD_EPL, a.cnsctvo AS CONSECUTIVO,a.fec_solicitud AS FECHA_SOL,
...
and a.cod_aus=(select descripcion from parametros_nue where nom_var='param_vacas_cod_aus')";
     ...
     return $this->lista;
        ...
        public function solicitud_epl_gral_rechazada(){
         ...
         //Sentencia sql del historico de contratos
         $codiepl = $_SESSION['cod'];
            $sql="select a.observacion AS OBSERVACION, a.cod_con AS COD_CON, a.cod_epl AS COD_EPL, a.fec_solicitud AS FECHASOL,a.cnsctvo AS CONSECUTIVO,e.cedula AS CEDULA,
 ...
 order by a.fec_ini desc";
     ...
     public function solicitud_pendientes_epl_gral(){
         ...
         $codiepl = $_SESSION['cod'];
            ...
            from empleados_basic emp, cargos car, centrocosto2 cen ,horasextras_tmp au, EMPLEADOS_GRAL b
            where
