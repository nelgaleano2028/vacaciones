...
<head>
	...
	if ($(this).val() == "oci8"){
	    $(".ora").css("display","table-row");
	}else{
	     $(".ora").css("display","none");
	}
	});
      });
	  ...
	  <table class="fondo_tabla_form" border="0" align="center" cellpadding="2" cellspacing="0" valign="top" width="100%">
		  <tbody>
			  ...
			  <option value="oci8">ORACLE</option>
		       ...
		       <tr class="ora" style="display: none">
			<td>Sid:</td>
			<td><input type="radio" name="ora" id="siec" value="1"/></td>
		       </tr>
		        <tr class="ora" style="display: none">
			<td>Service name:</td>
			<td><input type="radio" name="ora" id="service" value="2"/></td>
		       </tr>
		  <tr>
...
<?php
	  ...
	  if($_POST){
	      if($_POST["engine"]=='odbc_mssql'){
		   ...
		   $conexion='Connect($dsn,$user,$pass,"'.$_POST["db"].'")';
			...
			$oracle="SERVICE_NAME";
	                 ...
	                 $dns = '(DESCRIPTION=
			 (ADDRESS_LIST=
			 (ADDRESS = (PROTOCOL=TCP)(HOST = '.$_POST['host_db'].')(PORT = '.$_POST['host_port'].'))
			 )
			 (CONNECT_DATA =
			 ('.$oracle.' = '.$_POST['db'].')
			 )
	   ...
	   $archivo2 = "../lib/configdb.php"; //Archivo en el cual escribiremos
	   $datos = '<?php
	   ...
	   $fp2 = @fopen($archivo2,'w+');// Abrimos el archivo en forma de escritura (a)
	   @fwrite($fp2, $datos); // Colocamos los datos apuntando al archivo en gestor ($fp) con el contenido de $datos
	   @fclose($fp2); //Cerramos el modo de escritura en archivo $archivo
	   if(file_exists("../lib/connection.php") && file_exists("../lib/configdb.php")){
...
<br><br><center><a href="conceptos.php">Continuar con la instalaci&oacute;n.</a></center>
<?php
	   }else{
